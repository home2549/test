	...
	: QObject(parent),g_leftXOffset(0),g_rightXOffset(0)
{
	...
	ThirdDispatchMsg::reg_signal(this,SIGNAL(sig_dbg_v_rtls(const QByteArray &)),ThirdDispatchMsg::RTLS_V_TYPE);
...
void TrafficAlg::slot_vLastData(const LasterData & nDisInfo)
{
	...
	if(m_qCache.size()){
		if(m_qCache.size() > m_vCache.size() + 5){
			qDebug()<<"clear q data:"<<m_qCache.size() - m_vCache.size();
			//clear the q 
		...
		qDebug()<<"clear v self data:"<< m_vCache.size();
		qint32 nDel = 0;
		while(nDel < 10){
			m_vCache.dequeue();
			nDel++;
...
void TrafficAlg::slot_qLastData(const LasterData & nDisInfo)
{
	...
	if(m_vCache.size()){
		...
		if(m_vCache.size() > m_qCache.size() + 5){
			qDebug()<<"clear v data:"<<m_vCache.size() - m_qCache.size();
			//clear the q 
		...
		qDebug()<<"clear q self data:"<<m_qCache.size();
		qint32 nDel = 0;
		while(nDel < 10){
			m_qCache.dequeue();
			nDel++;
...
void TrafficAlg::process()
{
...
#ifdef DBG_LASTER	
	emit signal_VehicleDetect(l_FrameInfo1.u8Sum+l_FrameInfo2.u8Sum);
#endif

	bool bprint = false;
	if(bprint){
	qDebug()<<LMS_data_1.xdata();
	qDebug()<<LMS_data_1.zdata();
	}
	...
	if(TrafficSettings::instance()->getLasterV().LmsObstacleleft() && !g_leftXOffset)
	{
		...
		float ncos_val = m_vBackground->height()*1.0/m_qBackground->height();
		qDebug()<<"cos(x)"<<ncos_val;
	}
...
SINGLE_V_LABEL:
						...
						if(l_32tmp2 == 0)
						{
							...
							qDebug()<<"v first x:"<<l_leftX<<"("<<l_leftPt<<")"<<" y:"<<l_rightX<<"("<<l_rightPt<<")";
						}
						else
						{
...
SINGLE_Q_LABEL:
						...
						if(l_32tmp2 == 0)
						{
							...
							qDebug()<<"q first x:"<<l_leftX<<"("<<l_leftPt<<")"<<" y:"<<l_rightX<<"("<<l_rightPt<<")";
						}
						else
						{
							m_Vehicle[i]->locateX.u16xLeft = min(l_leftX,m_Vehicle[i]->locateX.u16xLeft);
...
//	qDebug()<<"final the len:"<<tmp1;
	//if( abs(TrafficSettings::instance()->getLasterQ().LmsHeight() - tmp1) < 1000 || tmp1 < 500) //偶尔会有小点，不清楚原因
	if(!tmp1)
	{
		tmp1 = pVeh->ZLen ? pVeh->ZLen : 1500;
	}
//	qDebug()<<"fix the len:"<<tmp1;

	tmp1 = tmp1*l_lenLasters/TrafficSettings::instance()->getLasterQ().LmsHeight();
...
//	qDebug()<<"fix the dis:"<<tmp1<<", calc dis:"<<l_lenLasters;
	...
	//qDebug()<<"back dis:"<<VehSpeed *(pVeh->Vdata.u16FrameCnt-1)*1000*10/(36*50);
	//计算车长		
	l_vTimer = pVeh->Vdata.tdata[0];
	...
	pVeh->yLen = pVeh->yLen*10/36;
		...
		//速度做补偿
	if(pVeh->Vdata.u16FrameCnt > 10){
		pVeh->speed = pVeh->speed* (pVeh->Vdata.u16FrameCnt+2)/pVeh->Vdata.u16FrameCnt;
//		qDebug()<<"raw speed:"<<VehSpeed<<" , fixed speed:"<<pVeh->speed;
//		qDebug()<<"raw len:"<< VehSpeed * abs(l_ITimer - l_vTimer)*10/36<<" , fixed len:"<<pVeh->speed * abs(l_ITimer - l_vTimer)*10/36;
	}	
	...
	VehPattern = GetVehPattern2(pVeh);
...
#if 1
uint16 TrafficAlg::GetVehPattern2(VehicleInfo *pVeh)
{
	...
	if(pVeh->Vdata.u16FrameCnt < 15){
	  //小货、中货、小客
		qDebug()<<"old method";
	   return GetVehPattern(pVeh);
	} else{		
		
		//车头前的玻璃干扰
		qint16 t_big_diff_pos = GetBigDiffPos(pVeh->Vdata.zMax,pVeh->Vdata.u16FrameCnt);
		if(t_big_diff_pos > 5){
			t_big_diff_pos = 1;
		}

		qint16 t_max_pos = GetMaxPos(pVeh->Vdata.zMax,pVeh->Vdata.u16FrameCnt - 2,t_big_diff_pos);
		qint16 t_min_pos =GetMinPos(pVeh->Vdata.zMax,pVeh->Vdata.u16FrameCnt - 2,t_big_diff_pos);

		qint32 t_zLen_mean = GetMean(pVeh->Vdata.zMax,0,pVeh->Vdata.u16FrameCnt);

	/*	if(pVeh->Vdata.zMax[t_max_pos] - t_zLen_mean < 1000){
			t_zLen_mean =(t_zLen_mean*7)>>3;
		}*/
		//不考虑第一个点和最后一个点
		qint16 tStart = GetContinueBigThanVal(pVeh->Vdata.zMax,pVeh->Vdata.u16FrameCnt - 1,t_zLen_mean,1);

				
		qDebug()<<"new method: mean--"<<t_zLen_mean<<" start:"<<tStart
			    <<" bigdiff:"<<pVeh->Vdata.zMax[t_max_pos] - pVeh->Vdata.zMax[t_min_pos]
				<<" point:"<<t_max_pos<<"---"<<t_min_pos;

		if(tStart == -1 || pVeh->Vdata.zMax[t_max_pos] - pVeh->Vdata.zMax[t_min_pos] > 600){
			//车头高、车身底一定是货：小货、中货、大货、特大
			//车不平，一定是货
			if(pVeh->ZLen < 2000){		
				if(pVeh->yLen >6000){
			...
			} else if(pVeh->ZLen < 3000){				
				if(pVeh->yLen > 10000){
					return TYPE_TE_DA_HUO;		
				} else if(pVeh->yLen < 8500){
					return TYPE_ZHONG_HUO;
				} else {
					return TYPE_DA_HUO;
				}
...
uint16 TrafficAlg::simpleVehPattern(VehicleInfo *pVeh)
{
	...
	if((t_vFrameUseless*100/pVeh->Vdata.u16FrameCnt) > 60 || pVeh->ZLen < 1600) // 2018/5/5  || pVeh->ZLen < 1600
	{		
...
int16 TrafficAlg::GetBigDiffPos(int32 *pArray,int32 nlen,int16 nstart)
{
	int16 retPos = 0;
	int32 t_MaxDiff = 0;
	int16 t_pos = nstart;
...
int16 TrafficAlg::GetMaxPos(int32 *pArray,int16 nlen,int16 nstart)
{
	int16 retPos = 0;
	int32 t_max = 0;
	int16 t_pos = nstart;
...
int16 TrafficAlg::GetMinPos(int32 *pArray,int16 nlen,int16 nstart)
{
	int16 retPos = 0;
	int32 t_min = 0;
	int16 t_pos = nstart;
	for(;t_pos<nlen;t_pos++)
	{
		if(pArray[t_pos] < t_min && pArray[t_pos] > FLY_MIN_Z_VAL)
		{
			t_min = pArray[t_pos];
			retPos = t_pos;
		}
...
void TrafficAlg::submit_v_rtls(const LasterData & ndata)
{
	QList<qint32> tHeights = ndata.data();
	
	QByteArray _bufer;
	DevProtocol::serialize(tHeights,_bufer);

	emit sig_dbg_v_rtls(_bufer);
}

void TrafficAlg::submit_q_rtls(const LasterData &)
{

}

qint16 TrafficAlg::GetLessthanValue(int32 *pArray,qint16 nlen,int32 nVal,qint16 nstart)
{
	int32 t_max_val = 0;
	qint16 t_max_index = -1;

	for(qint16 tIndex = nstart;tIndex<nlen;tIndex++){
		if(pArray[tIndex] < nVal && t_max_val < pArray[tIndex]){
			t_max_val = pArray[tIndex];
			t_max_index = tIndex;
		}
	}
...
qint16 TrafficAlg::GetRLessthanValue(int32 *pArray,qint16 nlen,int32 nVal,qint16 nstart)
{
	int32 t_max_val = 0;
	qint16 t_max_index = -1;

	qint16 tIndex = nstart;
	if(nstart == -1){
		tIndex = nlen - 1;
	}
...
qint16 TrafficAlg::GetRBigDiffPos(int32 *pArray,int32 nlen)
{
	int16 retPos = 0;
	int32 t_MaxDiff = 0;
	int16 t_pos = nlen - 1;
...
qint16 TrafficAlg::GetMinPos(int32 *pArray,int32 nlen,qint16 nstart)
{
	int16 retPos = 0;
	int32 t_min = 100000;
	int16 t_pos = nstart;
	for(;t_pos < nlen;t_pos++)
	{
		if(pArray[t_pos] < t_min && pArray[t_pos] > FLY_MIN_Z_VAL) //过滤飞点
		{
			t_min = pArray[t_pos];
			retPos = t_pos;
		}
...
qint16 TrafficAlg::GetContinueBigThanVal(int32 *pArray,int32 nlen,int32 nVal,qint16 nstart)
{
	int32 t_max_val = 0;
	qint16 t_index = -1;

	for(qint16 tIndex = nstart;tIndex<nlen;tIndex++){
		if(pArray[tIndex] < nVal){			
			t_index = tIndex;
		}
	}
